...
class FileStore:
    ...
    def __init__(self, base_dir: str | None = None):
        raw_dir = Path(config.store.raw_dir.replace("\\", "/"))
    ...
    def save_raw(
        ...
        Returns:
            从项目根起的相对路径，如 ``"data/files/raw/pdfs/3f9a2c81.pdf"``。
        """
    ...
    def _resolve(self, relative_path: str) -> Path:
        ...
        # _rel 产出的是项目根相对路径（含 data/files/raw 前缀），
        # 需剥离该前缀再以 self.base 为锚解析；processed 等非 raw 子树
        # 不带此前缀，直接从 _PROJECT_ROOT 解析
        raw_prefix = config.store.raw_dir.replace("\\", "/") + "/"
    ...
    def _rel(self, path: Path) -> str:
        ...
        当 ``data_dir`` 配置为绝对路径（外部存储）时，文件不在项目根下，直接返回绝对路径。
        """
        effective = self.effective_root.resolve()
        # 外部存储优先：文件不在 effective_root 下时返回绝对路径
        if not path.is_relative_to(effective):
            return path.as_posix()
        rel = path.relative_to(effective).as_posix()
        raw_prefix = config.store.raw_dir.replace("\\", "/").lstrip("/") + "/"
        if rel.startswith(raw_prefix):
            return rel
        # processed 等非 raw 子树，直接返回 effective-root 相对路径
        return rel
