...
> 原 V0.1 → V0.5 的横向分层计划与实际开发顺序偏离较大，roadmap 已于 2026-08-30
> 按 git 记录重写为「已完成时间线 + 待完成建议顺序」。已完成部分不再套 V0.x 编号。

```mermaid
flowchart LR
    DONE["已完成<br/>底座 + 存储 4 表<br/>摄入闭环 + 检索闭环"] --> V06["V0.6 摄入扩容"]
...
12. **检索不分"题目/知识点"意图** —— 两种 chunk 在同一 Collection 混合召回，LLM 综合组织（搜题目可总结方法，搜方法配例题）；Leader 意图集合 question/review/report/browse/ingest/manage（2026-08-28 意图内联 Leader，ROUTER 子 Agent 已删；2026-09-03 加 manage），不拆 knowledge
13. **TeamAgent 多 Agent 协作（主架构）** —— 一个 Leader 自由委派**查询侧 4 个 + 摄入侧 5 个**专业子 Agent：查询侧（搜索信息/VLM 理解/聚合数据/输出整理）读数据，摄入侧（文档识别/结构识别/题目维护/入库决策/错题管理）写数据（意图识别非独立成员，2026-08-28 内联 Leader 系统提示词，见下 12 条修订）；Leader 按需决定调谁/调几个/顺序，非固定模板。与 AlgoNotes 单 Agent 形成对比点。**已实测验证（2026-08-12，官方 examples/team）**：自由委派/多轮记忆/指令约束均通过；Leader prompt 必须写清完成标准 + 成员调用上限 + prompt 自洽。**2026-08-25 更新：GraphAgent 备用方案已移除**（trpc-agent 源码确认 TeamAgent 可用；`docs/agent/graph_fallback.md` 已删，REPORT_GEN 逻辑迁至 `docs/agent/retrieval/aggregate.md`）
13a. **摄入侧 Agent 化（2026-08-13 补充）** —— 摄入分两入口：**批量摄入**（ima 导出 PDF）走 CLI 脚本 `scripts/ingest.py`（开发者初始化）；**即时摄入**（学生 QQ 发照片/作业）走摄入侧 Agent（文档识别 → 结构识别 → 题目维护 → 入库决策，标为「错题」的题目再走错题管理写错因），回显确认由 Leader 完成（2026-08-28 起回显归 Leader，13a 初稿曾写「入库决策 Agent 完成」，已同步修订）——学生侧唯一的资料录入入口
