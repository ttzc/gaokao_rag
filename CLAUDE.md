...
12. **检索不分"题目/知识点"意图** —— 两种 chunk 在同一 Collection 混合召回，LLM 综合组织（搜题目可总结方法，搜方法配例题）；ROUTER 只分 question/review/report/browse，不拆 knowledge
13. **TeamAgent 多 Agent 协作（主架构）** —— 一个 Leader 自由委派 5 个专业子 Agent（意图识别/搜索信息/VLM 理解/聚合数据/输出整理）；Leader 按需决定调谁/调几个/顺序，非固定模板；GraphAgent 降级为备用方案。与 AlgoNotes 单 Agent 形成对比点。**已实测验证（2026-08-12，官方 examples/team）**：自由委派/多轮记忆/指令约束均通过；Leader prompt 必须写清完成标准 + 成员调用上限 + prompt 自洽
