...
学生: 生成周报
Bot: 📊 数学学习周报（8.4 - 8.10）
     本周新增错题 12 道 | 已掌握 4 道 | 掌握率 33% | 错因待补 5 道
     薄弱知识点 Top 3:
     1. 导数应用（恒成立）4 道 —— ↑ 恶化
     2. 圆锥曲线（离心率）3 道 —— 持平
     3. 立体几何（二面角）2 道 —— 新增
...
### 错因待补与补录（2026-09-17 新增）

批量入库时用户往往没逐题口述错因，故错题本允许「**先记下来、后补**」：`errors` 行先建成**待补**状态，
**查询碰到时由 Leader 提示补充**（不催、不阻塞）：

```text
学生: 我的错题本
Bot: 共 12 道错题（已掌握 4 道）：
     1.【圆锥曲线】椭圆焦点三角形面积最值
     2.【导数应用】恒成立参数取值范围（错因待补）
     3.【立体几何】二面角余弦值计算
     ...
     其中 3 道还没写错因——想补的话直接说一句就行（如「第 2 题我公式记混了」）

学生: 第 2 题我是分离参数没想起来
Bot: 已记下第 2 题的错因：
     ▸ 错误类型：思路错误
     ▸ 错因：未想到用分离参数法处理恒成立问题
     ▸ 建议：复习「分离参数法」专题，配合同类题 3 道
```

> 录入时直接交代错因（见上「错题录入」「文档上传处理」）与事后补录**两条路径等价**——
> 区别只是错因在写入时一并落库，还是后续经 `update_error` 补上。
...
Bot: 已识别到 3 道题目：
     ...
     想直接说错因也行（如「2 题公式记混了」），不说也可以——回头随时补
