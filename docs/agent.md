...
**检索侧配合（树展开上卷）**：查询侧不走 Agent 调树——检索时直接调 `store/db/topics.py` 的 `expand_tag_names(node)`，取子树所有节点 name+aliases 并集作为过滤词，交给 `AgenticLangchainKnowledgeSearchTool` 对 `metadata.topic_tags` 做匹配（数组 `$contains` + `$or`，格式见 [store/vector/vector_store.md「Metadata 格式与过滤语义」](store/vector/vector_store.md)），实现"问圆锥曲线 → 搜到椭圆/双曲线/抛物线的题"。
