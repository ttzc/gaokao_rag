...
> 与 Chroma metadata 的关系：本表存**结构化关联**（SQLite 精确过滤），`topic_tags`（名字快照）存**检索用快照**（语义过滤，`$contains` + 树展开上卷）——两者同构（都存名字），摄入时同事务写入。格式见 [data_model.md「Metadata 设计」](../../data_model.md)。
