...
存储讲义/学习资料中的**知识点讲解段**（概念、公式、典型方法），本质是**纯文本 RAG**——比带图的题目简单，不需要 VLM，文本切块向量化即可。用户问"什么是分离参数法"→ 命中 `knowledge_point` chunk → 返回讲解 + 关联例题。
...
- **纯文本路线**：content 向量化 → `knowledge_point` chunk（`chunk_type` 之一），`doc_id` 桥接 SQLite ↔ Chroma
- **`topic_id` 可空**：识别不出知识点时先挂 NULL，后续归位补挂（不给摄入流程添堵）
- **检索价值**：复习建议可链接到具体讲解（"先看圆锥曲线讲义：分离参数法"）

## 常见操作

- 插入：`INSERT` + 同步写 Chroma `knowledge_point` chunk（同事务）
- 按 topic 查：`WHERE topic_id = ?` 或经树展开（父节点 → 全部子孙的讲解）
- 删除：删行 + 删对应 Chroma chunk（`doc_id` 关联）
...
| Chroma | `doc_id` ↔ `knowledge_point` chunk |
