...
- **`has_image` 是 Chroma 过滤专用**：Chroma metadata 存 `has_image` 布尔快照（bool 标量过滤最直接，避免检索时回查 SQLite 才能判断含图）；**SQLite 侧不存该字段**——以 `image_file_ids` 为准（非空即含图），避免两边维护不一致。摄入时同步写 Chroma metadata。Chroma metadata **不存 `image_file_ids`**（SQLite 权威，检索用不上，见 vector.md「Metadata 格式与过滤语义」）
