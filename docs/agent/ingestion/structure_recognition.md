...
| `pending_questions` | 题目清单（每题：一句话概括 + 题目 / 答案 / 解析三段 + 关联图像 / 来源（若有，来源=Skill 的 source_hint 原样保留）；**不留原文块**） | 题目维护 Agent 标注知识点 → Leader 回显 → 入库决策写库（来源行拆解映射 exam_year/question_number/exam_regions） |
