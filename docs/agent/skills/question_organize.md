...
## 输出（对齐 questions 表四要素）

| 字段 | 必填 | 说明 |
|------|------|------|
| `question` | ✅ | 规范、可作答的数学题面（含已知条件、设问） |
| `answer` | ✅ | 参考答案（含要点）；无答案时显式 `null` 并标注 |
| `explanation` | ✅ | 分步解析，每步标注知识点（可空串但需显式） |
...
- **不编造**：缺失的答案 / 解析绝不硬编，置 `null` 并写清「缺什么」，交由入库决策回显让学生补全。
...
实际 `SKILL.md`（含 YAML frontmatter 与完整 prompt）由 Claude 在 `src/agent/skills/question_organize/SKILL.md` 实现；本文件仅为功能说明。
