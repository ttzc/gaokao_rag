...
- [ ] **验证 TeamAgent 可用性**（`trpc_agent_sdk.teams` 的 Leader 委派 API；若当前版本不可用，退回 GraphAgent——见 [Agent 设计](agent_design.md)）
...
- [ ] 复习建议节点（基于错题统计）
- [ ] **周报/月报节点**：`periodic_reports` 表 + 周期聚合 + LLM 针对性练习建议（见 [Agent 设计](agent_design.md) REPORT_GEN 节点）
