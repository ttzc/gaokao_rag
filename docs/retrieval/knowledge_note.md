...
1. `hybrid_search(query, k, where)` 返回框架 `SearchResult`（见 [retriever.md](retriever.md)）
...
> 与 `question.search_questions` 共用 `hybrid_search` 底层，只是后过滤的 `doc_type` 不同——题目与讲解在同一 Collection 联合召回，由 Agent 综合。
