...
利用 tRPC-Agent 的 `LangchainKnowledge` + `LangchainKnowledgeSearchTool`（MVP 纯向量相似度检索，不配过滤条件）；后续需要 metadata 条件检索时升级 `AgenticLangchainKnowledgeSearchTool`（LLM 自动构建 `KnowledgeFilterExpr`，示例见 [retrieval/knowledge.md「框架集成」](retrieval/knowledge.md)）。
