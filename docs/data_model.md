...
存储试卷/作业/专题/错题中的**题目完整信息**（四要素：题目内容含图片描述 / 答案解析 / 错题错因 / 知识点关联）。题目文本、答案、解析存于本表（**SQLite 自包含**——离线可查），同时拆分 3 种 chunk 入 Chroma（question / answer / knowledge_point，`doc_id` 桥接）做语义检索——双写，本表是权威源。**可重建内容（VLM 描述、原始提取文本）不占本表**，存 `processed/`（vlm_desc/、text/）经哈希关联（见 [processed.md](store/files/processed.md)）。
