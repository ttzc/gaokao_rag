...
2. **错题录入：VLM 只识别题目，错因靠用户口述**：潦草手写识别不可靠（CER 15-20%），因此设计上**不存手写解题过程**——VLM 仅负责识别题目本身（回显确认），错因由用户口述 + LLM 结构化总结（见 [errors 表](store/db/errors.md) `error_summary`），绕开手写识别的准确率瓶颈
